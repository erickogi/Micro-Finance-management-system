...
private void check(){
     ...
     // if(jd.equals("")){
     //      JOptionPane.showMessageDialog(null, "Enter date man","Warning",JOptionPane.WARNING_MESSAGE);
     // }
     ...
     String query = "INSERT INTO loans("
      ...
      String []list={ "Cash", "Mpesa", "Cheque"};
       ...
       Double bal=Double.valueOf(fee)-Double.valueOf(appfe);
        n.addToOrgAccount(Double.valueOf(appfe),"Loan Application Fee" ,mode);
        if(bal>0){
             JOptionPane.showMessageDialog(null, "Balance is " +bal.toString());
              if(n.executeSQlQuery(query, "Processed")==1){
             n.RemoveFromOrgAccount(Double.valueOf(txtAmount.getText()),"LOAN GIVEN",jComboBoxMode.getSelectedItem().toString());
             deleteApplication(lblPersonId.getText());
       }
        }
        else if(bal<0){
